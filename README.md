...
| `Conflict. The container name … is already in use` | Fixed — `run.sh` now removes a stopped leftover automatically, and refuses (without killing it) if a run is genuinely still up. On an older checkout: `docker rm -f saber-replication-run`. |
